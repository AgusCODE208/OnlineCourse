...
class CourseService
{
    ...
    public function enrollUser(Course $course)
    {
        ...
        if (!$course->courseStudent()->where('user_id', $user->id)->exists()) {
    ...
    {
        $firstSectionId = $course->courseSection()->orderBy('id')->value('id');
        $firstContentId = $firstSectionId
            ? $course->courseSection()->find($firstSectionId)->sectionContent()->orderBy('id')->value('id')
            : null;
