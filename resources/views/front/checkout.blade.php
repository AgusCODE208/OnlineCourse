...
@section('title','Checkout - Agus Kurnia')
@section('content')
...
<main class="flex flex-1 justify-center py-5 items-center">
    <div class="flex w-[1000px] !h-fit rounded-[20px] border border-obito-grey gap-[40px] bg-white items-center p-5">
        <form method="POST" id="checkout-details"  class="w-full flex flex-col gap-5">
            @csrf
            ...
            <section id="transaction-details" class="flex flex-col gap-[12px]">
                ...
                <div class="flex flex-col gap-[12px]">
                    ...
                    <div class="flex items-center justify-between">
                        ...
                        <strong class="font-bold text-[22px] leading-[33px] text-obito-green">Rp {{  number_format($grant_total_amount, 0, '', '.') }}</strong>
                    </div>
                </div>
            </section>
...
<script type="text/javascript" src="https://app.sandbox.midtrans.com/snap/snap.js" data-client-key="{{ config('midtrans.clientKey') }}">console.log("Midtrans Client Key:", "{{ config('midtrans.clientKey') }}");</script>
